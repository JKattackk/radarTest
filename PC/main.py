...
ser = serial.Serial('COM4', baudrate=115200, bytesize=8, parity='N', stopbits=1, timeout=2)  # serial part
...
ax.set_thetagrids(np.linspace(-60, 60, 13))

angles = np.arange(0, 120, 1)
theta = angles * (np.pi / 120)
...
while True:
    try:
        ...
        data = ser.readline()
        if data == b'':
            print("no message received or invalid format")
        else:
            print(data)
            decoded = data.decode()
            # decoded = data
