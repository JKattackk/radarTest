...
int main(void) {
    ...
    while(1){
        ...
        printf("Greatest Frequency Component C1: %0.1f Hz with power %0.2f \n",max_freqCW, max_power);
        
        printf("standard deviation is %.2f \n", standardDev(power));
        if((max_power-mean) > (20*standardDev(power)))
        {
            printf("signal present\n");
        }else
        {
            printf("signal not present\n");
        }
